...
<section class="content-header">
  <h1>
    <i class="fa fa-desktop icon-title"></i> Portafolio
  ...
  <div class="row">
    ...
    elseif ($_GET['alert'] == 6) {
      echo "<div class='alert alert-danger alert-dismissable'>
              <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
              <h4>  <i class='icon fa fa-times-circle'></i> Error!</h4>
              Asegúrese de que el archivo cargado está en *.JPG / *.JPEG / *.PNG. / *.MP4.
            </div>";
    }
    ?>
            ...
            while ($data = mysqli_fetch_assoc($query)) { 
              ...
              echo "<tr>
                      ...
                      <td class='center' width='80'>
                        <div>
                          <a data-toggle='tooltip' data-placement='top' title='Editar' style='margin-right:5px' class='btn btn-primary btn-sm' href='?module=form_portfolio&form=edit&id=$data[portfolio_id]'>
                              <i style='color:#fff' class='glyphicon glyphicon-edit'></i>
                          </a>";
            ?>
                          <a data-toggle="tooltip" data-placement="top" title="Eliminar" class="btn btn-danger btn-sm" href="modules/portfolio/proses.php?act=delete&id=<?php echo $data['portfolio_id'];?>" onclick="return confirm('Estas seguro que quieres eliminar <?php echo $data['title']; ?> ?');">
